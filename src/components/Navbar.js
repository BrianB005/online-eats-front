...
const Navbar = () => {
  ...
  return (
    ...
    <Wrapper fixed={isScrolled}>
      <NavContent>
        <Left>
          <SideBarToggle onClick={()=>openSideBar()}><FaBars/></SideBarToggle>
          <Link to="/"><Logo>OnlineEats</Logo></Link>
        </Left>
        <Center>
          <SearchContainer>
            <Input placeholder="Search dish/drink by name or category" autoFocus onChange={(e)=>setSearchTerm(e.target.value)}/>
            <SearchIcon><FaSearch/></SearchIcon>
          </SearchContainer>
        </Center>
        <Links>
          <Link to="/about"><NavBarLink>About</NavBarLink></Link>
          <Link to="/contact"><NavBarLink>Contact</NavBarLink></Link>
          {userInfo?<NavBarLink onClick={signout}>Logout</NavBarLink>:
          <div style={{display:"flex"}}>
          <Link to="/login"><NavBarLink>Login</NavBarLink></Link>
          <Link to="/register"><NavBarLink>Register</NavBarLink></Link>
          </div>}
          <Link to="/account"><NavBarLink>Account</NavBarLink></Link>
        </Links>
        <Right>
          <Cart>
...
const Wrapper=styled.div`
  background:  ${props=>props.fixed==="true"?"lightseagreen":"#B2F9FC"};
  height: 55px;
